...
final class PhotosViewModelImplementation: PhotosViewModel {
    ...
    private func bindOnViewDidLoad() {
        viewDidLoad
            ...
            .do(onNext: { [unowned self] _ in self.getPhotos() })
            .subscribe()
        ...
        willDisplayCellAtIndex
            ...
            .compactMap ({ [weak self] (index, photo) in
                guard let urlString = photo.urls?.regular else {
                    DispatchQueue.main.async { self?.imageRetrievedError.accept(index) }
                    return nil
                }
            ...
            .subscribe(onNext: { [weak self] (index, data, error) in
                ...
                guard let imageData = data,
                    let image = self.dataToImageService.getImage(from: imageData) else {
    ...
    private func bindOnDidEndDisplayCell() {
    ...
    private func bindPageNumber() {
        pageNumber
            .subscribe(onNext: { [weak self] _ in self?.getPhotos() })
            .disposed(by: disposeBag)
    }
    
    private func bindOnDidChoosePhoto() {
        didChoosePhotoWithId
            .subscribe(onNext: { [unowned self] id in self.coordinator.pushToPhotoDetail(with: id) })
            .disposed(by: disposeBag)
    }
